...
#             out_cpp.dec_indent()        
    ...
    def gen_cmake(self, ast):
        ...
        out.println()
        out.println("# Leave the MSVC runtime library selection to the parent build.  Forcing")
        out.println("# the static CRT here would give this library its own heap and its own")
        out.println("# copy of the iostream/locale state, which breaks as soon as C++ objects")
        out.println("# or allocations cross the boundary to a /MD consumer such as a Python")
        out.println("# extension module.")
        out.println()
...
#        out.println("add_library(" + self.name + "_static ${" + self.name + "_SRC})")
        ...
        out.println(")")
        out.println()
        out.println("# Selects the dllexport side of %s_FACTORY_API in FactoryExt.h." % self.name.upper())
        out.println("# PRIVATE: consumers of the installed headers must see the dllimport side.")
        out.println("target_compile_definitions(%s PRIVATE %s_BUILD_DLL)" % (
