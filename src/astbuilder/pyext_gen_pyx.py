...
#        self.gen_defs(self.pyx)
        ...
        self.pyx.inc_indent()
        self.pyx.println("_libname = \"lib%s.dylib\"" % self.name)
        self.pyx.dec_indent()
        self.pyx.println("elif sys.platform == 'win32':")
        self.pyx.inc_indent()
        self.pyx.println("_libname = \"%s.dll\"" % self.name)
        self.pyx.dec_indent()
        self.pyx.println("else:")
        self.pyx.inc_indent()
        self.pyx.println("_libname = \"lib%s.so\"" % self.name)
        self.pyx.dec_indent()
    ...
    def gen_defs(self, out):
        ...
        if self.namespace is not None:
            ...
            out.println("cdef extern from \"%s\":" %
                CppGenNS.incpath(self.namespace, "impl/UP.h"))
        out.inc_indent()
    ...
    def gen_factory(self):
        ...
        self.pyx.println("libname = _libname")
        self.pyx.println("core_lib = None")
        self.pyx.println("for libdir in (\"lib\", \"lib64\", \"bin\"):")
        self.pyx.inc_indent()
        ...
        self.pyx.inc_indent()
        self.pyx.println("core_lib = os.path.join(ext_dir, libname)")
        self.pyx.dec_indent()
        ...
        self.pyx.dec_indent()
        self.pyx.println("if _libname == \"lib%s.dylib\":" % self.name)
        self.pyx.inc_indent()
        self.pyx.println("import glob as _glob")
        self.pyx.println("for _al in _glob.glob(os.path.join(os.path.dirname(core_lib), 'libantlr4-runtime*.dylib')):")
        self.pyx.inc_indent()
        self.pyx.println("ctypes.cdll.LoadLibrary(_al)")
        self.pyx.dec_indent()
        self.pyx.dec_indent()
